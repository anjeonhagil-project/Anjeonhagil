...
if(process.argv.includes('--db')){
    ...
    check(result.status===0&&s?.table_count===16&&s?.active_release?.length===1&&s?.active_model?.length===1&&s?.unvalidated_constraints?.length===0&&s?.rls_disabled?.length===0&&s?.local_release_matches&&s?.local_model_matches,'원격 DB: 16개 서비스 테이블·활성 데이터/모델 해시 일치·제약·RLS')
