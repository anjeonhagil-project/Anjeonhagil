...
with connect() as db:
    try:
        ...
        db.execute(sql)
        assert db.execute('SELECT count(*) n FROM pg_tables WHERE schemaname=%s',[schema]).fetchone()['n']==38
        passed+=1
    ...
    passed+=1
print(json.dumps({'passed':passed,'scope':'38-table fresh bootstrap, PostGIS, RLS, auth trigger, preferences, model, refusal, rollback'}))
