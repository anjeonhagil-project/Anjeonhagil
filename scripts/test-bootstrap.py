...
with connect() as db:
    try:
        ...
        db.execute(sql)
        assert db.execute('SELECT count(*) n FROM pg_tables WHERE schemaname=%s',[schema]).fetchone()['n']==39
        passed+=1
    ...
    passed+=1
print(json.dumps({'passed':passed,'scope':'39-table fresh bootstrap, PostGIS, RLS, auth trigger, preferences, model, Q4 profile, refusal, rollback'}))
