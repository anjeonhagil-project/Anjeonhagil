...
let id,browser,page,noticeId;let passed=10;const errors=[],consoleErrors=[],failures=[]
...
try{
    ...
    // SPA sidebar navigation must not render the previous route's response under the new kind.
    await page.getByRole('link',{name:'경로 검색 기록',exact:true}).click()
    await page.getByRole('main').getByRole('heading',{name:'경로 검색 기록',exact:true}).waitFor()
    ...
    await page.getByRole('heading',{name:'도로 데이터',exact:true}).waitFor();passed++
    ...
    for(let i=0;i<4;i++){
        ...
        await page.getByRole('heading',{name:'데이터·모델 관리',exact:true}).waitFor()
        await page.getByRole('heading',{name:'도로 데이터',exact:true}).waitFor()
        await page.getByRole('heading',{name:'추천 모델',exact:true}).waitFor()
        passed++
    }
    ...
    const report={passed,errors,consoleErrors,failures,scope:'real admin browser, repeated dataset loads, chart periods, filters, dialogs, unpublished draft and isolated inquiry; temporary records cleaned'}
    writeFileSync('.test-tools/ui-results/admin-report.json',JSON.stringify(report,null,2));console.log(report)
}catch(error){if(page){await page.screenshot({path:'.test-tools/ui-results/admin-failure.png',fullPage:true}).catch(()=>{});writeFileSync('.test-tools/ui-results/admin-failure.json',JSON.stringify({message:error.message,url:page.url(),errors,consoleErrors,failures},null,2))}throw error}
