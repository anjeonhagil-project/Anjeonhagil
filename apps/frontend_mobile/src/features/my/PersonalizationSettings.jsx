...
export default function PersonalizationSettings(){
    ...
    const [confirmRestart,setConfirmRestart]=useState(false),[editing,setEditing]=useState(false)
    ...
    async function restart(){setBusy(true);setError('');try{await apiClient.post('/driving-preferences/q4/restart',{});setConfirmRestart(false);setEditing(true)}catch(e){setError(e.message)}finally{setBusy(false)}}
    async function closeSurvey(){setEditing(false);try{setState(await apiClient.get('/driving-preferences/personalization'))}catch(e){setError(e.message)}}
    if(editing)return <section className="route-card"><h2>시간·거리 경로 비교 설문</h2><p className="service-note">기존 부담 설정과 행동 보정은 유지됩니다. 답변은 문항별로 저장됩니다.</p><button onClick={closeSurvey}>나중에 이어서 하기</button><RouteChoiceStep onComplete={closeSurvey}/></section>
    return <section className="route-card" style={{padding:16,marginBottom:20}}>
        ...
        {state&&<>
            ...
            {state.q4?.applicable&&<label style={{display:'flex',gap:10,marginBottom:12}}><input type="checkbox" checked={state.q4?.enabled??false} disabled={busy} onChange={e=>q4Change(e.target.checked)}/>이전 설문 결과를 추천에 참고</label>}
            {state.q4?.pending||state.q4?.status==='INCOMPLETE'?<button disabled={busy} onClick={()=>setEditing(true)}>{state.q4?.pending?'시간·거리 설문 이어서 하기':'시간·거리 선호 설문 시작'}</button>:!confirmRestart?<button disabled={busy} onClick={()=>setConfirmRestart(true)}>시간·거리 선호 다시 설정</button>:<div><p className="service-note">부담 순위와 행동 보정, 이전 응답은 유지됩니다. 새 응답은 학습 준비용으로 저장하며 추천에는 아직 적용하지 않습니다.</p><button disabled={busy} onClick={restart}>새 설문 시작</button> <button disabled={busy} onClick={()=>setConfirmRestart(false)}>취소</button></div>}
