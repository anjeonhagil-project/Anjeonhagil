...
export default function RouteChoiceStep({onComplete}) {
    ...
    return (
        <>
        <div ref={content} className={`service-content hide-scrollbar ${styles.step}`} data-testid="q4-step">
            ...
            <p className={styles.intro}><b>{factor?.label}</b> 부담과 {q.dimension==='TIME'?'이동 시간':'이동 거리'}를 비교해주세요.</p>
            ...
            {/* <div className={styles.difference} aria-label="두 경로의 차이"><b>{q.dimension==='TIME'?`시간 차이 ${timeGap}분`:`거리 차이 ${distanceGap.toLocaleString()}m`}</b></div> */}
            ...
            <button className={choice==='UNSURE'?`${styles.unsure} ${styles.selected}`:styles.unsure} aria-pressed={choice==='UNSURE'} disabled={busy} onClick={()=>setChoice('UNSURE')}>상관없음</button>
        ...
        <div className={styles.footer}>
            <Button fullWidth disabled={!choice||busy} onClick={next}>{busy?'저장 중…':index===3?'설정 완료':'다음 문항'}</Button>
        </div>
        </>
    )
}
