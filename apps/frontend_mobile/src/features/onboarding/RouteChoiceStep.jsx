...
export default function RouteChoiceStep({onComplete}) {
    ...
    return <div ref={content} className="service-content q4-step">
        <p className="service-note">경로 비교 설문 {index+1} / 4</p>
        <div className="q4-progress"><span style={{width:((index+1)*25)+'%'}}/></div>
        <h2 className="q4-title">어느 길을 이용하고 싶나요?</h2>
        <p className="q4-intro"><b>{factor?.label}</b> 부담과 {q.dimension==='TIME'?'이동 시간':'이동 거리'}를 비교해주세요. 정답은 없어요.</p>
        ...
        <RouteMap comparison candidates={routes} selectedId={routes.find(r=>r.label===choice)?.candidate_id} onSelect={id=>{if(!busy)setChoice(routes.find(r=>r.candidate_id===id).label)}} origin={q.origin} destination={q.destination} height={200}/>
        <div className="q4-map-legend"><span>● 경로 A</span><span>┄ 경로 B</span></div>
        <div className="q4-difference" aria-label="두 경로의 차이"><b>{q.dimension==='TIME'?`시간 차이 ${timeGap}분`:`거리 차이 ${distanceGap.toLocaleString()}m`}</b><span>부담 차이 {metricValue(Math.abs(a.raw_features[factorIndex]-b.raw_features[factorIndex]),factorIndex)}</span></div>
        <div className="q4-options">{routes.map(r=><button key={r.label} className={'q4-option'+(choice===r.label?' selected':'')} aria-pressed={choice===r.label} disabled={busy} onClick={()=>setChoice(r.label)}>
            <span className={'q4-card-heading route-'+r.label}><b>경로 {r.label}</b><span aria-hidden="true">{choice===r.label?'✓':'○'}</span></span>
            <strong className="q4-time">{durationLabel(r.display_duration_s)}</strong><span className="q4-distance">{distanceLabel(r.distance_m)}</span>
            <span className="q4-focus"><span>{factor?.label}</span><b>{metricValue(r.raw_features[factorIndex],factorIndex)}</b></span>
        </button>)}</div>
        <p className="q4-unit-note">부담 수치는 낮을수록 해당 요소가 적습니다. {factorIndex===5?'시설 중심 100m 원 안을 지나는 거리이며, 법정 보호구역 전체를 뜻하지 않습니다.':factorIndex===1||factorIndex===2?'점수·m는 도로별 부담 점수에 통과 거리를 곱한 값입니다.':'지표는 경로에서 계산한 부담값이며 사고 확률이 아닙니다.'}</p>
