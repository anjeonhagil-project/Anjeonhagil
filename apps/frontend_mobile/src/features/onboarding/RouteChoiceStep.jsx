...
export default function RouteChoiceStep({onComplete,completionLabel='설정 완료',embedded=false,onBusyChange,footerTarget}) {
    ...
    if(!q)return <div className="service-content hide-scrollbar">{error?<p role="alert">{error}<button type="button" onClick={()=>setAttempt(v=>v+1)}>다시 시도</button></p>:<p role="status">비교 설문을 불러오고 있어요…</p>}</div>
    ...
    const footer=<div className={styles.footer} data-testid="q4-footer"><Button type="button" fullWidth disabled={!choice||busy} onClick={next}>{busy?'저장 중…':index===3?completionLabel:'다음'}</Button></div>
    return <><div ref={content} className={`service-content hide-scrollbar ${styles.step} ${embedded?styles.embedded:''}`} data-testid="q4-step">
        ...
        <button type="button" className={choice==='UNSURE'?`${styles.unsure} ${styles.selected}`:styles.unsure} aria-pressed={choice==='UNSURE'} disabled={busy} onClick={()=>setChoice('UNSURE')}>상관없음</button>
        ...
        <p className="service-note">다음으로 넘어가면 답변이 저장돼요. 저장한 답변은 설문을 다시 시작해 변경할 수 있어요.</p>
    ...
    </div>{embedded?(footerTarget?createPortal(footer,footerTarget):null):footer}</>
}
