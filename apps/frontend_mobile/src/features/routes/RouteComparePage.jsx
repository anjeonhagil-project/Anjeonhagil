...
export default function RouteComparePage() {
    ...
    useEffect(()=>{
        ...
        if(!promiseRef.current){abortRef.current=new AbortController();promiseRef.current=savedId?getSearch(savedId):searchRoutes(inputRef.current,{signal:abortRef.current.signal})}
        promiseRef.current.then(data=>{
            if(!active)return
            setResult(data);setSelected(data.selectedCandidateId||null);setChosen(!!data.selectedCandidateId)
        ...
        return ()=>{active=false;cleanupRef.current=setTimeout(()=>abortRef.current?.abort(),0)}
    },[attempt])
    ...
    async function choose(){
        ...
        try {if(!frozenChoice.current)frozenChoice.current={selected,exposure:await exposure.record(interaction.current)};await recordRouteChoice(frozenChoice.current.exposure.exposureId,{choiceEventId:choiceRef.current,selectedCandidateId:frozenChoice.current.selected});setChosen(true)}
        catch(e){setError(e.message)}finally{setSaving(false)}
    }
    const [expanded,setExpanded]=useState(true)
    const [focusEvent,setFocusEvent]=useState(null)
    function select(id,source){if(saving||frozenChoice.current)return;interaction.current={selectionSource:source,selectionChanges:interaction.current.selectionChanges+(selected&&selected!==id?1:0)};setSelected(id);setFocusEvent(null);setExpanded(true);if(source==='map')requestAnimationFrame(()=>document.querySelector(`[data-candidate-id="${id}"]`)?.scrollIntoView({block:'center',behavior:'smooth'}))}
    ...
    return <main className="service-page compare-page journey-wide">
        ...
        <div className="service-content compare-layout">
            ...
            {result&&!loading&&<>
                ...
                <p className="service-note">출발·도착은 주변의 주행 가능한 도로에 연결됩니다.{result.originSnap&&` 출발 연결 ${Math.round(result.originSnap.distance_m||0)}m`}{result.destinationSnap&&` · 도착 연결 ${Math.round(result.destinationSnap.distance_m||0)}m`}. 연결 구간의 보행·진입 가능 여부는 확인이 필요합니다.</p>
                ...
                <div className="compare-footer">{!chosen?<><p aria-live="polite">{selectedCard?`${selectedCard.route_types.map(t=>ROUTE_LABELS[t]).join(' · ')} · ${durationLabel(selectedCard.display_duration_s)} · ${distanceLabel(selectedCard.distance_m)}`:'경로 카드를 확인하고 직접 선택해주세요.'}</p><button className="service-primary" disabled={!selected||!exposure.hasSeen(selected)||saving} onClick={choose}>{saving?'저장 중…':'이 경로 선택하기'}</button></>:<section className="route-success" role="status"><h2>경로 선택을 저장했어요</h2><p>선택한 경로로 GPS 안내 또는 시뮬레이션을 시작하세요.</p><button className="service-primary" onClick={()=>navigate('/navigation?search='+result.searchId)}>이 경로 안내 시작</button></section>}</div>
                </section>
            </>}
        </div>
    </main>
