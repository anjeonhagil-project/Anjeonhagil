    ...
    return <main className="service-page">
        ...
        <div className="service-content">
            ...
            {result&&!loading&&<>
                ...
                <RouteMap candidates={result.candidates} selectedId={selected} onSelect={chosen?undefined:setSelected} origin={result.origin} destination={result.destination}/>
                ...
                {!chosen?<button className="service-primary" disabled={!exposure.isRecorded||saving} onClick={choose}>{saving?'저장 중…':'이 경로 선택하기'}</button>:<section className="route-success" role="status"><h2>경로 선택을 저장했어요</h2><p>지도에서 전체 경로를 확인하세요. 이 서비스는 경로 비교용이며 실시간 주행 안내를 제공하지 않습니다.</p><button onClick={()=>navigate('/home')}>홈으로</button></section>}
