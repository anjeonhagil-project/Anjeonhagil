...
class FeatureCalculator:
 def __init__(self,features,transitions,graph):self.f=connect(features);self.t=connect(transitions);self.g=connect(graph);self.params={k: json.loads((Path(__file__).resolve().parent/'rules.json').read_text())['parameters'][v] for k,v in {'complex_node_degree_min':'complex_degree','multilane_directional_min':'directional_lanes','turn_angle_min_deg':'unfamiliar_angle','action_angle_min_deg':'action_angle','consecutive_max_distance_m':'consecutive_distance_m'}.items()}
