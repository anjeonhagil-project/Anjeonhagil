...
def search(
    ...
    *,
    ...
    while queue:
        ...
        goal_allowed = end_follow_arc is None or (
            (previous_arc == -1 or any(a == end_follow_arc for a, _, _ in trans.get(previous_arc, ())))
            and rules.advance(history, end_follow_arc) is not None
        )
        if node == t and goal_allowed and (
