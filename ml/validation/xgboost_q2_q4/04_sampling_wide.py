"""Q2+Q4 XGBoost 4단계: subsample × colsample_bytree 넓은 범위 검증.

프로젝트 루트에서 실행:
    python -B ml/validation/xgboost_q2_q4/04_sampling_wide.py

입력:
    ml/data/q2_q4/synthetic_pairwise_q2_q4.csv

3단계 결과:
    ml/artifacts/xgboost_q2_q4/03_lr_trees_fine/selected_config.json

출력:
    ml/artifacts/xgboost_q2_q4/04_sampling_wide/
...
STAGE3_CONFIG = (
    ...
    / "03_lr_trees_fine" / "selected_config.json"
)
...
COLSAMPLE_VALUES = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
...
def prepare_output(output: Path) -> None:
    ...
    if existing:
        ...
        raise FileExistsError(
            "기존 04_sampling_wide 결과가 있습니다. 자동으로 덮어쓰지 않습니다.\n"
            f"{files}\n"
...
def load_stage3_config():
    if not STAGE3_CONFIG.is_file():
        raise FileNotFoundError(
            "3단계 selected_config.json을 찾을 수 없습니다.\n"
            f"예상 경로: {STAGE3_CONFIG}\n"
            "03_lr_trees_fine.py를 먼저 실행하세요."
        )

    config = json.loads(STAGE3_CONFIG.read_text(encoding="utf-8-sig"))
    ...
    if missing:
        raise ValueError(
            f"3단계 selected_config.json에 필수 값이 없습니다: {missing}"
        )

    if config["test_evaluated"] is not False:
        raise ValueError(
            "3단계에서 Test 미평가 상태를 확인할 수 없습니다."
        )

    stage3_depth = int(config["max_depth"])
    stage3_lr = float(config["learning_rate"])
    stage3_trees = int(config["n_estimators"])

    if stage3_depth != FIXED_MAX_DEPTH:
        raise ValueError(
            "3단계 max_depth와 4단계 고정값이 다릅니다.\n"
            f"3단계: {stage3_depth}\n"
            f"4단계: {FIXED_MAX_DEPTH}"
        )
    ...
    if not np.isclose(
        ...
        raise ValueError(
            "3단계 learning_rate와 4단계 고정값이 다릅니다.\n"
            f"3단계: {stage3_lr}\n"
            f"4단계: {FIXED_LEARNING_RATE}"
        )

    if stage3_trees != FIXED_N_ESTIMATORS:
        raise ValueError(
            "3단계 n_estimators와 4단계 고정값이 다릅니다.\n"
            f"3단계: {stage3_trees}\n"
            f"4단계: {FIXED_N_ESTIMATORS}"
        )
...
def search_top1(model, frame, scale):
    ...
    for _, group in frame.groupby(
        ...
        pair_x = (
            reconstruct_all_pair_differences(group)
            / scale
        )
...
def evaluate(model, frame, scale):
    ...
    reverse_raw = model.predict_proba(
        -x
    )[:, 1]
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Q2+Q4 XGBoost 4단계: "
            "subsample × colsample_bytree 넓은 범위 검증"
        )
    )
    ...
    print(
        "[XGBoost Q2+Q4 "
        "subsample × colsample_bytree 4차 넓은 검증]"
    )
    ...
    report = {
        "stage":
            "sampling_wide_q2_q4_validation",
        "comparison_scope":
