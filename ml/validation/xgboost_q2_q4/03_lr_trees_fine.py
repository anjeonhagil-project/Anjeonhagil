"""Q2+Q4 XGBoost 3단계: learning_rate × n_estimators 세부 검증.

프로젝트 루트에서 실행:
    python -B ml/validation/xgboost_q2_q4/03_lr_trees_fine.py

입력:
    ml/data/q2_q4/synthetic_pairwise_q2_q4.csv

2단계 결과:
    ml/artifacts/xgboost_q2_q4/02_lr_trees_wide/selected_config.json

출력:
    ml/artifacts/xgboost_q2_q4/03_lr_trees_fine/

규칙:
- max_depth=9 고정
- learning_rate = 0.040, 0.045, 0.050, 0.055, 0.060, 0.065, 0.070
- n_estimators = 200, 220, 250, 300, 350, 400
- 총 42조합 비교
...
- 정확한 동률이면 더 적은 n_estimators -> 더 작은 learning_rate
"""
...
STAGE2_CONFIG = (
    ...
    / "02_lr_trees_wide" / "selected_config.json"
)
...
def prepare_output(output: Path) -> None:
    ...
    if existing:
        ...
        raise FileExistsError(
            "기존 03_lr_trees_fine 결과가 있습니다. 자동으로 덮어쓰지 않습니다.\n"
            f"{files}\n"
...
def load_stage2_config():
    if not STAGE2_CONFIG.is_file():
        raise FileNotFoundError(
            "2단계 selected_config.json을 찾을 수 없습니다.\n"
            f"예상 경로: {STAGE2_CONFIG}\n"
            "02_lr_trees_wide.py를 먼저 실행하세요."
        )

    config = json.loads(
        STAGE2_CONFIG.read_text(encoding="utf-8-sig")
    )
    ...
    if missing:
        raise ValueError(
            f"2단계 selected_config.json에 필수 값이 없습니다: {missing}"
        )

    if config["test_evaluated"] is not False:
        raise ValueError(
            "2단계에서 Test 미평가 상태를 확인할 수 없습니다."
        )

    stage2_depth = int(config["max_depth"])
    stage2_lr = float(config["learning_rate"])
    stage2_trees = int(config["n_estimators"])

    if stage2_depth != FIXED_MAX_DEPTH:
        raise ValueError(
            "2단계에서 선택된 max_depth와 3단계 고정값이 다릅니다.\n"
            f"2단계: {stage2_depth}\n"
            f"3단계 고정값: {FIXED_MAX_DEPTH}"
        )

    return config, stage2_lr, stage2_trees
...
def load_data(input_path: Path):
    ...
    if not set(df["split"].unique()).issubset(allowed_splits):
        raise ValueError(
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Q2+Q4 XGBoost 3단계: "
            "learning_rate × n_estimators 세부 검증"
        )
    )
    ...
    stage2_config, stage2_lr, stage2_trees = (
        load_stage2_config()
    )
    ...
    print(
        "[XGBoost Q2+Q4 "
        "learning_rate × n_estimators 3차 세부 검증]"
    )
    ...
    print(
        "2단계 선택:",
        f"learning_rate={stage2_lr}, "
        f"n_estimators={stage2_trees}",
    )
    ...
    print(
        "고정: subsample=1.0, colsample_bytree=1.0"
    )
    ...
    report = {
        "stage":
            "lr_trees_fine_q2_q4_validation",
        "comparison_scope":
        ...
        "stage2_config_sha256":
            sha256(STAGE2_CONFIG),
        "features":
            FEATURES,
        "fixed_max_depth":
            FIXED_MAX_DEPTH,
        "stage2_selected_learning_rate":
            stage2_lr,
        "stage2_selected_n_estimators":
            stage2_trees,
        "fixed_params":
            FIXED_PARAMS,
