"""Q2+Q4 XGBoost 5단계: subsample × colsample_bytree 세부 검증.

프로젝트 루트에서 실행:
    python -B ml/validation/xgboost_q2_q4/05_sampling_fine.py

입력:
    ml/data/q2_q4/synthetic_pairwise_q2_q4.csv

4단계 결과:
    ml/artifacts/xgboost_q2_q4/04_sampling_wide/selected_config.json

출력:
    ml/artifacts/xgboost_q2_q4/05_sampling_fine/

규칙:
- max_depth=9, learning_rate=0.06, n_estimators=300 고정
...
- 정확한 동률이면 더 큰 subsample -> 더 큰 colsample_bytree
"""
...
STAGE4_CONFIG = (
    ...
    / "04_sampling_wide" / "selected_config.json"
)
...
SUBSAMPLE_VALUES = [0.85, 0.90, 0.95, 1.00]
COLSAMPLE_VALUES = [0.70, 0.80, 0.90]
...
def prepare_output(output: Path) -> None:
    ...
    if existing:
        ...
        raise FileExistsError(
            "기존 05_sampling_fine 결과가 있습니다. 자동으로 덮어쓰지 않습니다.\n"
            f"{files}\n"
...
def load_stage4_config():
    if not STAGE4_CONFIG.is_file():
        raise FileNotFoundError(
            "4단계 selected_config.json을 찾을 수 없습니다.\n"
            f"예상 경로: {STAGE4_CONFIG}\n"
            "04_sampling_wide.py를 먼저 실행하세요."
        )

    config = json.loads(STAGE4_CONFIG.read_text(encoding="utf-8-sig"))
    ...
    if missing:
        raise ValueError(
            f"4단계 selected_config.json에 필수 값이 없습니다: {missing}"
        )

    if config["test_evaluated"] is not False:
        raise ValueError(
            "4단계에서 Test 미평가 상태를 확인할 수 없습니다."
        )

    stage4_depth = int(config["max_depth"])
    stage4_lr = float(config["learning_rate"])
    stage4_trees = int(config["n_estimators"])
    stage4_subsample = float(config["subsample"])
    stage4_colsample = float(config["colsample_bytree"])

    if stage4_depth != FIXED_MAX_DEPTH:
        raise ValueError(
            f"4단계 max_depth={stage4_depth}, "
            f"5단계 고정값={FIXED_MAX_DEPTH}로 서로 다릅니다."
        )

    if not np.isclose(stage4_lr, FIXED_LEARNING_RATE, rtol=0.0, atol=1e-12):
        raise ValueError(
            f"4단계 learning_rate={stage4_lr}, "
            f"5단계 고정값={FIXED_LEARNING_RATE}로 서로 다릅니다."
        )

    if stage4_trees != FIXED_N_ESTIMATORS:
        raise ValueError(
            f"4단계 n_estimators={stage4_trees}, "
            f"5단계 고정값={FIXED_N_ESTIMATORS}로 서로 다릅니다."
        )

    return config, stage4_subsample, stage4_colsample
...
def reconstruct_all_pair_differences(group):
    ...
    values = {
        int(rows[0].a): np.zeros(len(FEATURES), dtype=float)
    }
...
def evaluate(model, frame, scale):
    ...
    return {
        ...
        "raw_direction_error_mean": float(
            np.mean(np.abs(raw + reverse_raw - 1.0))
        ),
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Q2+Q4 XGBoost 5단계: "
            "subsample × colsample_bytree 세부 검증"
        )
    )
    ...
    _, stage4_subsample, stage4_colsample = load_stage4_config()
    ...
    total_combinations = (
        ...
        * len(COLSAMPLE_VALUES)
    )

    print(
        "[XGBoost Q2+Q4 "
        "subsample × colsample_bytree 5차 세부 검증]"
    )
    ...
    print(
        "4단계 선택:",
        f"subsample={stage4_subsample}, "
        f"colsample_bytree={stage4_colsample}",
    )
    ...
    selected_colsample = float(
        selected["colsample_bytree"]
    )
    ...
    report = {
        "stage": "sampling_fine_q2_q4_validation",
        "comparison_scope": "train_validation_only",
        "input_file": str(input_path),
        "input_sha256": sha256(input_path),
        "source_metadata_sha256": sha256(source_metadata_path),
        "stage4_config_file": str(STAGE4_CONFIG),
        "stage4_config_sha256": sha256(STAGE4_CONFIG),
        "features": FEATURES,
        ...
        "stage4_selected_subsample": stage4_subsample,
        "stage4_selected_colsample_bytree": stage4_colsample,

        "subsample_values": SUBSAMPLE_VALUES,
        "colsample_bytree_values": COLSAMPLE_VALUES,
        "total_combinations": total_combinations,

        "selection_rule": (
            "validation_log_loss_then_top1_"
            "then_higher_subsample_then_higher_colsample_bytree"
        ),
        ...
        "placement_seed":
            source_metadata.get("placement_seed"),
    ...
    save_json(
        output / "selected_config.json",
        {
            ...
            **FIXED_PARAMS,
            ...
            "stage4_config_sha256":
                report["stage4_config_sha256"],
    ...
    save_json(
        output / "metadata.json",
        {
            ...
            "training_source":
                source_metadata.get("training_source"),

            "real_user_validated":
                source_metadata.get("real_user_validated"),
    ...
    for row in results:
        ...
        result["selected"] = (
            row["subsample"] == selected_subsample
            and row["colsample_bytree"] == selected_colsample
        )

        comparison_rows.append(result)

    with (
        output / "sampling_fine_comparison.csv"
    ).open(
        "w",
        encoding="utf-8-sig",
        newline="",
    ...
    lines = [
        ...
        "- 정확한 동률: 더 큰 subsample",
        "- 이후 동률: 더 큰 colsample_bytree",
        "- Test는 평가하지 않음",
        "",
        "결과",
        (
            ...
            "Validation Top-1 | Validation ROC-AUC | "
            "Validation Accuracy"
        ),
    ]
    ...
    lines.extend([
        ...
        f"learning_rate: {FIXED_LEARNING_RATE}",
        f"n_estimators: {FIXED_N_ESTIMATORS}",
        f"subsample: {selected_subsample}",
        f"colsample_bytree: {selected_colsample}",
