"""Q2+Q4 XGBoost 1단계: max_depth 2~10 검증.

프로젝트 루트에서 실행:
    python -B ml/validation/xgboost_q2_q4/01_max_depth.py

입력:
    ml/data/q2_q4/synthetic_pairwise_q2_q4.csv

출력:
    ml/artifacts/xgboost_q2_q4/01_max_depth/

규칙:
- max_depth = 2, 3, 4, 5, 6, 7, 8, 9, 10 비교
- learning_rate=0.06, n_estimators=220 고정
...
- 선택 기준: Validation Log Loss -> Validation Top-1 -> 더 작은 max_depth
"""
...
DEFAULT_OUTPUT = ROOT / "ml" / "artifacts" / "xgboost_q2_q4" / "01_max_depth"
...
def prepare_output(output: Path) -> None:
    ...
    if existing:
        ...
        raise FileExistsError(
            "기존 01_max_depth 결과가 있습니다. 자동으로 덮어쓰지 않습니다.\n"
            f"{files}\n"
...
def infer_chosen_candidate(group):
    chosen = [int(row.a if row.y == 1 else row.b) for row in group.itertuples(index=False)]

    if len(set(chosen)) != 1:
        raise ValueError(
            f"search_id={group.iloc[0]['search_id']}의 pair들이 서로 다른 선택 후보를 가리킵니다."
        )
...
def evaluate(model, frame, scale):
    ...
    direction_error = float(np.max(np.abs(p + probability(model, -x) - 1.0)))
...
def main():
    parser = argparse.ArgumentParser(
        description="Q2+Q4 XGBoost 1단계: max_depth 2~10 검증"
    )
    ...
    print("[XGBoost Q2+Q4 max_depth 1차 검증]")
    print("입력:", input_path)
    ...
    print("고정: learning_rate=0.06, n_estimators=220, subsample=1.0, colsample_bytree=1.0")
    print("비교 max_depth:", DEPTH_VALUES)
    print("선택: Validation Log Loss -> Validation Top-1 -> 작은 max_depth")

    results = []
    models = {}

    for depth in DEPTH_VALUES:
        print(f"\nmax_depth={depth} 학습 중...", flush=True)

        params = {"max_depth": depth, **FIXED_PARAMS}
    ...
    selected_model = models[selected_depth]

    prepare_output(output)

    selected_model.save_model(output / "selected_model.json")

    restored = XGBClassifier()
    restored.load_model(output / "selected_model.json")
    ...
    report = {
        "stage": "max_depth_q2_q4_validation",
        ...
        "selection_rule": "validation_log_loss_then_top1_then_smaller_depth",
    ...
    for row in results:
        result = {"max_depth": row["max_depth"]}
        ...
        result["selected"] = row["max_depth"] == selected_depth
        comparison_rows.append(result)

    with (output / "depth_comparison.csv").open(
        "w",
        encoding="utf-8-sig",
        newline="",
    ) as file:
    ...
    lines = [
        ...
        "결과",
        (
            "max_depth | Train Log Loss | Validation Log Loss | "
            "Validation Top-1 | Validation ROC-AUC | Validation Accuracy"
        ),
    ]
